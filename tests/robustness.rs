...
fn test_edge_case_file_sizes() {
    ...
    let names: Vec<_> = matches.iter().map(|m| m.file_path.file_name().unwrap().to_str().unwrap()).collect();
...
fn test_repetitive_data_explosion() {
    ...
    let plan = Planner::plan("abc", false);
    let (matches, _) = executor.execute(&plan, &QueryOptions {
        max_results: 100,
        ..Default::default()
    }).unwrap();
...
fn test_context_merging_logic() {
    ...
    fs::write(root.join("context.txt"), "match\nmatch\nmatch\nother\nother\nmatch").unwrap();
