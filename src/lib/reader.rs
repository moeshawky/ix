...
impl Reader {
    ...
    pub fn get_trigram(&self, trigram: Trigram) -> Option<TrigramInfo> {
        ...
        let posting_length = entry.get(6..10)
            .and_then(|s| s.try_into().ok())
            .map(u32::from_le_bytes)
            .unwrap_or(0);
        let doc_frequency = entry.get(10..14)
            .and_then(|s| s.try_into().ok())
            .map(u32::from_le_bytes)
            .unwrap_or(0);
    ...
    pub fn get_file(&self, file_id: u32) -> Result<FileInfo> {
        ...
        let entry = self.mmap.get(entry_off .. entry_off + FILE_ENTRY_SIZE)
        ...
        let path_off = u32::from_le_bytes(entry[4..8].try_into().map_err(|_| Error::Config("invalid path offset".into()))?);
        let status = FileStatus::from_u8(entry[10]);
        let mtime_ns = u64::from_le_bytes(entry[12..20].try_into().map_err(|_| Error::Config("invalid mtime".into()))?);
        let size_bytes = u64::from_le_bytes(entry[20..28].try_into().map_err(|_| Error::Config("invalid size".into()))?);
        let content_hash = u64::from_le_bytes(entry[28..36].try_into().map_err(|_| Error::Config("invalid hash".into()))?);
    ...
    pub fn bloom_may_contain(&self, file_id: u32, trigram: Trigram) -> bool {
        ...
        if bloom_rel_off_bytes.is_none() { return true; }
        
        let bloom_rel_off = u32::from_le_bytes(bloom_rel_off_bytes.unwrap().try_into().unwrap_or([0u8; 4]));
        ...
        let size = u16::from_le_bytes(self.mmap[bloom_abs_off .. bloom_abs_off + 2].try_into().unwrap_or([0u8; 2])) as usize;
